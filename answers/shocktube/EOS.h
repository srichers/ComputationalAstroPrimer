...
class EOS{
 public:
  ...
  double internal_energy_density(double density, double pressure) const{
    assert(gamma>1.); // make sure EOS is initialized

    return pressure / (density * (gamma-1.));
  }

  double pressure(double density, double internal_energy_density) const{
    assert(gamma>1.); // make sure EOS is initialized

    return density * internal_energy_density * (gamma-1.);
  }

  double soundspeed(double density, double pressure) const{
    assert(gamma>1.);
